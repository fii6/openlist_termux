...
| 运维 | 开机自启、版本检测、数据备份/还原、全局快捷命令 `oplist` |

## 前置要求

- **Termux** (Android)
- **curl**：`pkg install -y curl`（脚本会自动检测并尝试安装）
- **termux-services**：`pkg install -y termux-services`（脚本首次启停服务时会自动尝试安装）
...
启停由 [termux-services](https://wiki.termux.com/wiki/Termux-services) 提供的 `runit` 接管，相比早期 `nohup` 方案的关键差异：

- **崩溃自动重启**：`runsv` 监督每个服务，进程退出会立刻拉起
- **统一接口**：`sv up/down/restart/status <name>` 替代散落的 `pkill -f`
- **集中日志**：每个服务由 `svlogd` 写入对应目录的 `current` 文件，按 size 自动轮转
- **单一开机自启脚本**：`~/.termux/boot/openlist_termux_services` 启动 `runsvdir`，一次拉起全部启用的服务

服务定义安装到 `$PREFIX/var/service/{openlist,aria2,cloudflared}/`，禁用自启时在该目录写入 `down` 文件，下次 `runsvdir` 启动后不会再拉起该服务。

日志位置：

| 服务 | svlogd 当前日志 |
|------|----------------|
| openlist | `~/Openlist/data/log/sv/current` |
| aria2 | `~/aria2/log/sv/current` |
| cloudflared | `~/.cloudflared/log/sv/current` |
